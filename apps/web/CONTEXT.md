...
shapes T09 confirmed — `/i/{slug}/claim`, `/session/turns`,
`/session/runs/{id}`, `/session/artifacts/{id}`, `/session/finish`,
`/auth/login`, `/admin/sessions`, `/admin/sessions/{id}` — not against
T09's code), not any Python package in this repository. Nothing here
imports `interviewer_core` or `interviewer_adapters`.
...
both `REPLY_MODE`s, no build flag.
